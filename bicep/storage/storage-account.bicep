...
resource storage 'Microsoft.Storage/storageAccounts@2022-05-01' = {
  ...
  sku: sku
  properties: {
    isHnsEnabled: isHnsEnabled // Hierarchical namespace enabled
    isNfsV3Enabled: isNfsV3Enabled // NFSv3 enabled
    ...
    networkAcls: {
      ...
      ipRules: (private && (!empty(clientIpAddress)) ) ? [
        {
          action: 'Allow'
...
module storagePrivateEndpoint '../network/private-endpoint.bicep' = [for privateDnsZoneName in privateDnsZoneNames: if ( private ) {
  ...
  params: {
    ...
    privateEndpointName: 'pe-${storageAccountName}-${privateDnsZoneName}'
